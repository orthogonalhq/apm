...
export function DeletePackage({
  ...
  const expected = `delete ${packageName}`;
  ...
  async function handleDelete() {
    ...
    const res = await fetch(`/api/packages/@${scopeName}/${packageName}`, {
      method: "DELETE",
    });
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleClose(); }}>
          ...
          <div className={`relative w-full max-w-md px-4 ${closing ? "animate-dialog-out" : "animate-dialog-in"}`} onClick={(e) => e.stopPropagation()}>
            <div className="border border-white/[0.06] rounded-lg bg-[#141414] p-4 space-y-4 shadow-2xl">
              <div>
                <p className="text-sm t-heading font-medium">Delete package</p>
                <p className="text-xs t-nav mt-1">
                  This will permanently delete <span className="font-mono text-red-400">@{scopeName}/{packageName}</span> from the registry.
                </p>
              </div>

              <div>
                <label className="block font-mono text-[10px] uppercase tracking-[0.15em] t-ghost mb-1.5">
                  Type <span className="t-meta">delete {packageName}</span> to confirm
                </label>
                <input
                  type="text"
                  value={confirm}
                  onChange={(e) => setConfirm(e.target.value)}
                  placeholder={`delete ${packageName}`}
