...
- **Partial failure.** If a dependency fails to install, the parent composite skill is still installed and a warning is logged. The missing dependency can be retried with `apm install`.
