...
```bash
apm publish [path] --scope <scope>
```

**What it does:**

1. Reads `SKILL.md` from the given path (or current directory)
2. Detects the git remote, branch, and relative path automatically
3. Sends the git pointer to the registry — the registry fetches and validates the SKILL.md
4. Upserts the package record under the specified scope

**Authentication:** Requires an `APM_TOKEN` environment variable or a token saved at `~/.apm/token`. Create tokens at [apm.orthg.nl/settings](https://apm.orthg.nl/settings).

**Example:**

```bash
$ apm publish --scope apm
apm Publishing @apm/init...
  → orthogonalhq/apm-skills:init/SKILL.md (ref: main)
apm Published @apm/init
```

**Dependencies must be published first.** If a composite skill references dependencies that don't exist in the registry, the publish fails with a list of missing packages.
...
| `apm publish [path] --scope <scope>` | | Publish a skill to the registry |
...
| `APM_TOKEN` | Authentication token for `apm publish`. Also reads from `~/.apm/token`. |
